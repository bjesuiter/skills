...
Test that a skill can actually be installed. Local or global test installs from this repo must target only the coding agents configured in JB's preferred agent-environment registry (`jb-agent-env`). Do not add extra agents ad hoc. Current configured agents are `codex`, `opencode`, and `pi`; do not install for `crush`, `devin`, or `goose` unless the registry is explicitly updated first.
